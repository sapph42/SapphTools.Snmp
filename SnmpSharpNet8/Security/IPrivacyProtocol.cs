...
public interface IPrivacyProtocol {
    ...
    ReadOnlySpan<byte> Decrypt(ReadOnlySpan<byte> cryptedData, Span<byte> key, int engineBoots, int engineTime, Span<byte> privacyParameters);
    ReadOnlySpan<byte> Encrypt(ReadOnlySpan<byte> unencryptedData, Span<byte> encryptionKey, int engineBoots, int engineTime, out byte[] privacyParameters);
