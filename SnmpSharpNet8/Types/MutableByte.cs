...
public class MutableByte : ICloneable, IComparable<MutableByte>, IComparable<byte[]> {
    private sealed class MutableBytePool : MemoryPool<byte> {
        private readonly ArrayPool<byte> _arrayPool = ArrayPool<byte>.Create();
        public override int MaxBufferSize => int.MaxValue;
        public override IMemoryOwner<byte> Rent(int minBufferSize = -1) {
            return new MutableByteOwner(_arrayPool, minBufferSize);
        }
    ...
    private sealed class MutableByteOwner(ArrayPool<byte> pool, int minBufferSize) : IMemoryOwner<byte> {
        private byte[]? _array = pool.Rent(minBufferSize);
    ...
    public MutableByte(byte[] buf1, byte[] buf2) {
        ...
        Memory<byte> combined = rented.AsMemory(0, len);
        buf1.AsSpan().CopyTo(combined.Span);
        buf2.AsSpan().CopyTo(combined.Span[buf1.Length..]);
        _buffer = new(new byte[len]);
        combined.CopyTo(_buffer)
